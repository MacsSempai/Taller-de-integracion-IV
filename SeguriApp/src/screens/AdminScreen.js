...
export default function AdminScreen() {
  ...
  const updateMaterialPrices = async (materialId, newPrice) => {
    try {
      await axios.put(`http://192.168.1.11:3000/api/materiales/${materialId}`, { price: newPrice });
      setMaterialId('');
      setNewPrice('');
  ...
  const listCasesByUser = async (userId) => {
    try {
      const response = await axios.get(`http://192.168.1.11:3000/api/casos/${userId}/usuario`);
  ...
  const fetchMaterials = async () => {
    try {
      const response = await axios.get('http://192.168.1.11:3000/api/materiales');
  ...
  const fetchRoles = async () => {
    try {
      const response = await axios.get('http://192.168.1.11:3000/api/rol');
