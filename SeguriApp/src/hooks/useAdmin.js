...
const useAdmin = (usuarioId) => {
  ...
  const fetchMaterials = async () => {
    try {
      const response = await axios.get('http://190.114.253.250:3000/api/materials');
  ...
  const createUser = async (newUser) => {
    try {
      const response = await axios.post('http://190.114.253.250:3000/api/users', newUser);
  ...
  const updateMaterialPrices = async (materialId, newPrice) => {
    try {
      const response = await axios.put(`http://190.114.253.250:3000/api/materials/${materialId}`, { price: newPrice });
  ...
  const listCasesByRole = async (roleId) => {
    try {
      const response = await axios.get(`http://190.114.253.250:3000/api/casos/role/${roleId}`);
      setCases(response.data);
    } catch (err) {
