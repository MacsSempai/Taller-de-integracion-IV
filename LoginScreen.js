...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.10:3000/api/users/login', {
        email,
        password,
      });
      ...
      // Guardar todos los datos del usuario en AsyncStorage
      await AsyncStorage.setItem('userData', JSON.stringify(response.data.user));
      console.log('Datos almacenados en AsyncStorage:', response.data.user);
  ...
  const fetchUserRole = async (usuarioId) => {
    try {
      const roleResponse = await axios.get(`http://192.168.1.10:3000/api/rol/${usuarioId}`); // Cambia esta URL según tu API
